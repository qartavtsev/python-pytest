...
def test_add_dynamic_att(parametr1, parametr2):
    ...
    with allure.step('Шаг 2: Создание вложения HTML.'):
        allure.attach(f'<h1>The {parametr1} has {parametr2} wheels.</h1>', name='HTML example', attachment_type=allure.attachment_type.HTML  )

    with allure.step('Шаг 3: Создание вложения TXT.'):
        allure.attach(f'The {parametr1} has {parametr2} wheels.', name='TXT example', attachment_type=allure.attachment_type.TEXT)
